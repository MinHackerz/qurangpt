import { NextRequest, NextResponse } from 'next/server';

export async function OPTIONS() {
  return new NextResponse(null, {
    status: 200,
    headers: {
      'Access-Control-Allow-Origin': '*',
      'Access-Control-Allow-Methods': 'GET, POST, PUT, DELETE, OPTIONS',
      'Access-Control-Allow-Headers': 'Content-Type, Authorization',
    },
  });
}

export async function GET(request: NextRequest) {
  try {
    const { searchParams } = new URL(request.url);
    const surah = searchParams.get('surah');
    const startAyah = searchParams.get('startAyah');
    const endAyah = searchParams.get('endAyah');

    if (!surah || !startAyah || !endAyah) {
      return NextResponse.json(
        { error: 'Surah, start ayah, and end ayah parameters are required' },
        { status: 400 }
      );
    }

    // Validate surah and ayah numbers
    const surahNum = parseInt(surah);
    const startAyahNum = parseInt(startAyah);
    const endAyahNum = parseInt(endAyah);

    if (isNaN(surahNum) || isNaN(startAyahNum) || isNaN(endAyahNum) || 
        surahNum < 1 || surahNum > 114 || startAyahNum < 1 || endAyahNum < startAyahNum) {
      return NextResponse.json(
        { error: 'Invalid surah or ayah numbers' },
        { status: 400 }
      );
    }

    // Calculate the global ayah numbers for the range
    const surahAyahCounts = [
      7, 286, 200, 176, 120, 165, 206, 75, 129, 109, 123, 111, 43, 52, 99, 128, 111, 110, 98, 135, 112, 78, 118, 64, 77, 227, 93, 88, 69, 60, 34, 30, 73, 54, 45, 83, 182, 88, 75, 85, 54, 53, 89, 59, 37, 35, 38, 29, 18, 45, 60, 49, 62, 55, 78, 96, 29, 22, 24, 13, 14, 11, 11, 18, 12, 12, 30, 52, 52, 44, 28, 28, 20, 56, 40, 31, 50, 40, 46, 42, 29, 19, 36, 25, 22, 17, 19, 26, 30, 20, 15, 21, 11, 8, 8, 19, 5, 8, 8, 11, 11, 8, 3, 9, 5, 4, 7, 3, 6, 3, 5, 4, 5, 6
    ];

    // Calculate global ayah numbers for the range
    let baseGlobalAyah = 0;
    for (let i = 0; i < surahNum - 1; i++) {
      baseGlobalAyah += surahAyahCounts[i];
    }

    // Try multiple reciters as fallbacks
    const reciters = [
      'ar.alafasy',
      'ar.abdulbasitmurattal', 
      'ar.abdurrahmaansudais',
      'ar.abdulsamad'
    ];
    
    let audioUrls: string[] = [];
    let usedReciter = '';
    
    // Try to get audio for each ayah in the range
    for (let ayahNum = startAyahNum; ayahNum <= endAyahNum; ayahNum++) {
      const globalAyahNumber = baseGlobalAyah + ayahNum;
      let audioFound = false;
      
      for (const reciter of reciters) {
        try {
          const audioUrl = `https://api.alquran.cloud/v1/ayah/${globalAyahNumber}/${reciter}`;
          
          const response = await fetch(audioUrl, {
            headers: {
              'User-Agent': 'QuranGPT/1.0',
              'Accept': 'application/json',
              'Cache-Control': 'no-cache',
            },
            signal: AbortSignal.timeout(8000), // 8 second timeout
          });

          if (response.ok) {
            const data = await response.json();
            
            if (data.data && data.data.audio) {
              audioUrls.push(data.data.audio);
              if (!usedReciter) usedReciter = reciter; // Use first successful reciter
              audioFound = true;
              break;
            }
          }
        } catch (error) {
          // Failed to fetch audio with this reciter, try next one
          continue;
        }
      }
      
      if (!audioFound) {
        // If we can't get audio for any ayah in the range, return error
        return NextResponse.json(
          { error: `Audio not available for ayah ${surahNum}:${ayahNum} from any reciter` },
          { status: 404 }
        );
      }
    }

    if (audioUrls.length === 0) {
      return NextResponse.json(
        { error: 'No audio URLs found for the specified range' },
        { status: 404 }
      );
    }

    // Return the audio URLs with metadata
    return NextResponse.json({
      success: true,
      audioUrls: audioUrls,
      surah: surahNum,
      startAyah: startAyahNum,
      endAyah: endAyahNum,
      totalAyahs: audioUrls.length,
      reciter: usedReciter
    }, {
      headers: {
        'Access-Control-Allow-Origin': '*',
        'Access-Control-Allow-Methods': 'GET, POST, PUT, DELETE, OPTIONS',
        'Access-Control-Allow-Headers': 'Content-Type, Authorization',
      },
    });

  } catch (error) {
    console.error('Audio range API error:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
  }
}
